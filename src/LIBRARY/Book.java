...
public class Book {
    ...
    static void showList(ArrayList books) {
        ...
        ArrayList<String> eachBook = new ArrayList<String>();
        ...
        for (int i = 0; i < books.size(); i++) {
            ...
            getDisplay((ArrayList) books.get(i), i + 1);
        }
        System.out.format("+--------+-------------+----------------+-----------+%n");

    }

    static void getDisplay(ArrayList item, int iteration) {
        String leftAlignFormat = "| %-6d | %-11s | %-14s | %-9s |%n";

        System.out.format(leftAlignFormat, iteration, item.get(0), item.get(2), item.get(1));
    }
